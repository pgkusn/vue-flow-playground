...
<template>
  <aside
    class="flex w-64 flex-shrink-0 flex-col gap-3 overflow-y-auto border-r border-[#dcdfe6] bg-white p-5"
  >
    <div
      v-for="node in nodeTypes"
      :key="node.type"
      class="sidebar-card flex items-center justify-between gap-1.5 rounded-lg border border-[#d4d7de] bg-white p-3 transition select-none hover:border-[#3b82f6] hover:shadow-sm"
      :draggable="true"
    ...
    >
      ...
      <!-- 拖曳把手（取自設計稿 2×3 點陣） -->
      <svg class="h-3.5 w-2 flex-shrink-0" viewBox="0 0 8 14" fill="#909399" aria-hidden="true">
